...
One more thing: every eval needs an "Evidence required" field that forces the scorer to quote specific text from the output. Without this, scoring drifts toward gut feel. See the template at the bottom of this guide.
...
Evidence required: [What the scorer must quote or point to in the output to justify their decision]
```

The "Evidence required" field is the most important addition. Without it, scoring drifts toward vibes. With it, every pass/fail decision is anchored to something concrete in the output.
...
Evidence required: Quote any illegible, truncated, or overlapping text. If none found, state "all [N] text elements verified legible"
```

Example:

```
EVAL 2: No AI filler phrases
Question: Is the output free of common AI padding phrases?
Pass: No instances of banned phrases anywhere in the text
Fail: Any occurrence of phrases like "certainly", "great question", "I'd be happy to", "it's worth noting"
Evidence required: Quote the exact phrase and its surrounding sentence. If clean, state "scanned [N] paragraphs, zero matches"
```
